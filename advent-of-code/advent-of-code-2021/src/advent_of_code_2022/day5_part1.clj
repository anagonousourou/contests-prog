(ns advent-of-code-2022.day5-part1
...
(defn move-crates [all-stacks action]
  (loop [nb-crates-to-move (:crates-count action) all-stacks-inner all-stacks]
    (if (zero? nb-crates-to-move)
      all-stacks-inner
      (recur (dec nb-crates-to-move) (-> all-stacks-inner
                                         (update (:to-stack action) (fn[stack] (conj stack  (peek (all-stacks-inner (:from-stack action))))))
                                         (update (:from-stack action) pop)
                                         )
             )
