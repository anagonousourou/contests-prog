...
function processFuelRecords(string inputFilePath, string outputFilePath) returns error? {
    ...
    if (entries is string[][]) {
        ...
        string[][] csvRows = consumptionsFinal.'map(consumption => [
            consumption.employeeId.toString(),
            consumption.gasFillUpCount.toString(),
            consumption.totalFuelCost.toString(),
            consumption.totalGallons.toString(),
            consumption.totalMilesAccrued.toString()
        ]);

        // _ = check io:fileWriteCsv(outputFilePath, csvRows); this line throws an exception, maybe a bug on ballerina side
        _ = check io:fileWriteLinesFromStream(outputFilePath, csvRows.'map(row => string:'join(",", ...row)).toStream());
    }
