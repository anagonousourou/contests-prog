...
service /travel on new http:Listener(9090) {
    ...
    resource function post arrangeTour(@http:Payload TourArrangement tour) returns error|http:Response {
        ...
        ServiceResponse airlineResponse = check airlineReservationEP->/reserve.post(resForAirline);
        ...
        Reservation resForHotel = resForAirline.clone();
        resForHotel.preference = tour.preference.accomodation;
        ServiceResponse hotelResponse = check hotelReservationEP->/reserve.post(resForHotel);
        ...
        Reservation resForCar = resForAirline.clone();
        resForCar.preference = tour.preference.car;
        ServiceResponse carResponse = check carRentalEP->/rent.post(resForCar);
        ...
        response.setPayload({message: "Congratulations! Your journey is ready!!"});
        return response;
