...
TriangleType kind(double x, double y, double z)
{
    ...
    bool isTriangle = x > 0 && y > 0 && z > 0 && perimeter - x >= x && perimeter - y >= y && perimeter - z >= z;
    if (!isTriangle)
    {
        throw new Exception("The dimensions do not form a valid triangle.");
    }
    auto nbside = [x, y, z].sort.uniq.count;
    if (nbside == 1)
    {
